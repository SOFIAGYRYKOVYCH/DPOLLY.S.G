...
#include <string.h>
#include <assert.h>
#include "DPoly.h"  //headers

static void _dpoly_allocate(dpoly* poly, int size)
{
/*
Allocates the memory for the polynomial coefficients _without_ initializing them.
Should be used only when the coefficients will be initialized immediately after the call
*/
    assert(poly && size >= 0);
    if (size == 0)
    {
...
{
/*
Error handler. Terminates the program after printing a diagnostic
*/
...
{
/*
A function for assertions to request a pointer to a valid dpoly structure.
*/
...
{
/*
Resizes the polynomial, adding zero zoefficients if the size is larger than the current one, or removing larger coefficients, if it is smaller.
*/
    assert(_dpoly_valid(poly) && size >= 0);
    if (size == 0)
    {
            /*
            Uses the fact that the `dpoly` structure is a valid zero-sized polynomial after calling `dpoly_destroy`
            */
        dpoly_destroy(poly);
    }
    else
...
/*
Reduces the allocated space for the polynomial, removing highest zero coefficients */
...
void dpoly_generate(dpoly* poly, int size)
{
    ...
    {
        /* choses random coefficients from -1000 to 1000 range */
...
{
/*
adds a term "coef * x^order" to the polynomial, resizing it if needed */
assert(_dpoly_valid(poly) && order >= 0);
    /* check if we need to resize the polynomial */
    if (order >= poly->size)
    {
        /* if we resize, increase the size at least by half of the previous size to avoid quadratic time, when building the polynomial from the lowest order */
...
*/
    ...
    for (const char* c = s; *c != 0; )
        ...
        /* the only coefficient without sign can be the first one for others we got an error */
        if (sign == 0 && ncoef > 0)
        {
            return ncoef;
        }
        ...
        const char* end;
        /* Read the double coefficient from the string Yes, strtod _requires_ the second parameter to be modifiable
           even when the first one is 'const char*' This was done to preserve existing non-const correct code
...
{
/*
Reads a line of arbitrary length from file. The line returned must be `free`d after use */
    assert(file);
    int size = 0;
    int len = 0;
    int last = 0;
    char* buf = NULL;
    do {
        /* Increase the buffer by either half its length, or BUFSIZ, whatever is larger */
        int dsize = size >> 1;
        if (dsize < BUFSIZ)
        {
            dsize = BUFSIZ;
        ...
        size += dsize;
        /*
        Increase the size of the string buffer When buf == NULL 'realloc(NULL, size)' is the same as 'malloc(size)'
        */
        ...
        /*
        Reads the remaining part of the string. Nonzero return may mean either reaching end of file, which is ok,
        or another error on which we call an error handler
        If the last symbol is '\n' we have finished reading the line otherwise we need to increase our buffer and continue reading*/
